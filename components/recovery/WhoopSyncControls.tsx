...
/**
 * WHOOP card footer: last-synced relative time, "Sync now" (forces a sync via
 * server action), and a small "Disconnect" with a confirm().
 */
import { useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import { RefreshCw, Unplug } from "lucide-react";
import Button from "@/components/ui/Button";
import { disconnectWhoop, syncWhoopNow } from "@/lib/actions/whoop";
...
export default function WhoopSyncControls({ lastSyncedAt }: { lastSyncedAt: string | null }) {
  ...
  const disconnect = () => {
    if (!window.confirm("Disconnect WHOOP? Already-synced data is kept.")) return;
    setError(null);
    startTransition(async () => {
