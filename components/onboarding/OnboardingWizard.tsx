 ...
 * Three skippable steps: program choice, starting body weight, WHOOP.
...
export default function OnboardingWizard({ starter }: { starter: StarterProgram | null }) {
  ...
  const stepBody = [
    ...
    // Step 3 — WHOOP
    <div key="whoop" className="space-y-3">
      <h2 className="font-display text-xl font-semibold tracking-tight text-text">
        Do you wear a WHOOP?
      </h2>
      <p className="text-sm leading-relaxed text-text-3">
        Connect it and the coach reads your recovery each morning — on rough days it suggests
        backing off instead of pushing, and your briefs reference real sleep and strain. No
        WHOOP? Skip this and the app simply coaches from your training data.
      </p>
          ...
          {last ? (
            <div className="flex flex-col gap-2 sm:flex-row">
              <Button
                size="lg"
                disabled={pending}
                onClick={() => finish({ connectWhoop: true })}
                className="flex-1"
              >
              ...
              >
                ...
                {pending ? "Finishing…" : "I don't use WHOOP — finish"}
              </Button>
            </div>
          ) : (
